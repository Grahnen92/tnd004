...
Set<T> Set<T>::_union(const Set& b) const
{
	...
	while ( t1 < counter)
	{
		while (currB->value <= currNode->value && t2 < b.counter)
		{
			if (currB->value < currNode->value)
			{
				//newSet.insert(currNode, currB->value);
				Node* newNode = new Node;
				newNode->value = currB->value;
	...
	while (currB->value > tail->value && t2 < b.counter )
	{
		//newSet.insert(currNode, currB->value);
		Node* newNode = new Node;
		newNode->value = currB->value;
