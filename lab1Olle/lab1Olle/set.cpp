...
Set<T>::Set (const Set& b)
{
	...
	Node* currB = b.head->next;

	while (currB->next != nullptr)
	{
		insert(tail, currB->value);
		currB = currB->next;
	}
}
...
bool Set<T>::operator<=(const Set& b) const
{
	...
	while (currB->next != nullptr && currentNode->next != nullptr)
	{
		if (currentNode->value == currB->value)
		{
			testCounter++;
			currentNode = currentNode->next;
			currB = currB->next;
		}
		else if (currentNode->value > currB->value)
		{
			currB = currB->next;
		}
		else if (currentNode->value < currB->value)
...
bool Set<T>::operator==(const Set& b) const
{
	...
	while (currentNode->next != nullptr && currB->next != nullptr)
	{
		if (currentNode->value != currB->value)
		{
			return false;
...
Set<T> Set<T>::_union(const Set& b) const
{
	...
	Node* currentNode = newSet.head->next;
	Node* currB = b.head->next;

	while (currentNode->next != nullptr)
	{
		while (currB->value <= currentNode->value && currB->next != nullptr)
		{
			if (currB->value < currentNode->value)
				newSet.insert(currentNode, currB->value );
	...
	while (currB->value > currentNode->prev->value && currB->next != nullptr)
	{
		newSet.insert(currentNode, currB->value);
		currB = currB->next;
	}
...
Set<T> Set<T>::_intersection(const Set& b) const
{
	...
	while (currentNode->next != nullptr)
	{
		...
		{
			if (currB->value == currentNode->value)
				newSet.insert(newSet.tail, currB->value);
...
Set<T> Set<T>::_difference(const Set& b) const
{
	...
	if (b.counter != 0)
	{
		...
		while (currentNode->next != nullptr)
		{
			while (currB->value <= currentNode->value && currB->next != nullptr && currentNode->next != nullptr)
			{
				if (currB->value == currentNode->value)
				{
					temp = currentNode->next;
					newSet.erase(currentNode);
					currentNode = temp;
				}
		...
		while (currB->value > currentNode->prev->value && currB->next != nullptr)
		{
			if (currB->value == currentNode->prev->value)
				newSet.erase(currentNode->prev);
